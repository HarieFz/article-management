...
export default function DialogDelete({ id, name }: { id: string; name: string }) {
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: (data: { id: string }) => {
      return CategoriesService.delete(data.id);
    },
    onSuccess: () => {
    ...
    <>
      <Dialog open={open} onOpenChange={setOpen}>
        ...
        <DialogContent showCloseButton={false}>
          <DialogHeader>
            <DialogTitle className="font-semibold text-xl text-slate-900">Delete Category</DialogTitle>
